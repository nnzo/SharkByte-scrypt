...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("sharkbyte"))
        return false;
...
{
    // Convert sharkbyte:// to sharkbyte:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("sharkbyte://"))
    {
        uri.replace(0, 12, "sharkbyte:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "sharkbyte.lnk";
}
...
{
    return GetAutostartDir() / "sharkbyte.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("sharkbyte-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  sharkbyte-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("sharkbyte-Qt"));
